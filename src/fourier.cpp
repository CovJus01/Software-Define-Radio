...
void estimatePSD(std::vector<float> &freq, std::vector<float> &psd_est, const std::vector<float> &samples, int Fs){
	int freq_bins = NFFT;
	float df = static_cast<float>(Fs)/static_cast<float>(freq_bins);
	freq.clear();
	...
	for(int i = 0; i < freq_bins; i++){
		//hann.push_back(pow(sin(i*PI/freq_bins),2));
	...
	psd_list.reserve((freq_bins/2)*no_segments); // Reserve space for this list
	psd_seg.clear();
	...
	for(int k = 0; k < no_segments; k++){
		...
		// Crop the output or fill remaining space with zeroes
		Xf.resize(freq_bins, 0);

		Xf = std::vector<std::complex<float>>(Xf.begin(), Xf.begin() + static_cast<int>(freq_bins/2)); // keep only positive freq bins
		for(unsigned int i = 0; i < Xf.size(); i++){
			psd_seg.push_back((1.0/(static_cast<float>(Fs*freq_bins)/2.0)) * pow(abs(Xf[i]), 2)); // compute signal power
	...
	for(int k = 0; k < (int) freq_bins/2; k++){
		...
		psd_est[k] = static_cast<float>(psd_est[k]) / static_cast<float>(no_segments);
	}
